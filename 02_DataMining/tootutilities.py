...
def print_toot(toot):
    ...
    # if the language is not English, display original & translated text 
        ...
        # translate content
        try:
            content = BeautifulSoup(toot.content, 'html.parser').get_text() # remove HTML
...
def get_toot_content(toot):
    ...
    if toot.language and not toot.language.startswith('en'):
        try:
            plain_text = BeautifulSoup(toot.content, 'html.parser').get_text()
            result = translator.translate_text(plain_text, target_lang='en-us')
